...
const PRINT_BRAND = [
  ...
  { name: 'Light grey (print)', hex: '#f5f5f5', note: 'Collateral on white' },
]
...
function Swatch({ name, varName, hex, role, lightTheme = false }) {
  return (
    <div className={`hoverable ${lightTheme ? 'bg-white border border-slate-200 shadow-sm' : 'glass-card'}`} style={{ padding: '18px', borderRadius: '16px', transition: 'all 0.5s cubic-bezier(0.16, 1, 0.3, 1)' }}>
      <div
        style={{
          height: 76,
          borderRadius: 12,
          ...
          marginBottom: 14,
          boxShadow: lightTheme ? 'inset 0 0 0 1px rgba(0,0,0,0.05)' : 'inset 0 0 0 1px rgba(255,255,255,0.06)',
        }}
      />
      <div style={{ fontSize: '0.8125rem', fontWeight: 700, color: lightTheme ? '#0f172a' : '#ffffff', marginBottom: 6, letterSpacing: '-0.02em' }}>{name}</div>
      <code style={{ fontSize: '0.6875rem', color: lightTheme ? '#64748b' : '#7e99a8', display: 'block', marginBottom: 6, wordBreak: 'break-all' }}>{varName}</code>
      <div style={{ fontSize: '0.75rem', color: lightTheme ? '#475569' : '#b8cdd6', fontFamily: 'ui-monospace, monospace', marginBottom: 8 }}>{hex}</div>
      <div style={{ fontSize: '0.6875rem', color: lightTheme ? '#94a3b8' : '#3a5a6c', lineHeight: 1.45 }}>{role}</div>
    </div>
  )
}

function CodeBlock({ children, lightTheme = false }) {
  return (
    <pre
      style={{
        margin: '16px 0 0',
        padding: '16px 18px',
        borderRadius: 12,
        background: lightTheme ? '#f8fafc' : 'var(--color-midnight-light)',
        border: lightTheme ? '1px solid #e2e8f0' : '1px solid rgba(255,255,255,0.06)',
        fontSize: '0.75rem',
        lineHeight: 1.55,
        color: lightTheme ? '#334155' : '#b8cdd6',
        overflowX: 'auto',
...
function SectionTitle({ id, eyebrow, title, children, lightTheme = false }) {
  return (
    <header id={id} style={{ marginBottom: 40, scrollMarginTop: 100 }}>
      {eyebrow && (
        <p style={{ color: lightTheme ? '#d46b1a' : '#d46b1a', fontSize: '0.6875rem', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 12 }}>
          {eyebrow}
        </p>
      )}
      <h2 style={{ fontSize: 'clamp(1.75rem, 3vw, 2.5rem)', fontWeight: 800, color: lightTheme ? '#0f172a' : '#ffffff', letterSpacing: '-0.03em', lineHeight: 1.15, marginBottom: children ? 16 : 0 }}>
        {title}
      </h2>
      {children}
    </header>
...
export default function BrandDesignPage() {
  ...
  const toc = [
    ...
    { id: 'components', label: 'Light Theme UI' },
  ...
  return (
    <main style={{ position: 'relative', minHeight: '100vh', backgroundColor: 'var(--color-midnight)', overflow: 'hidden' }}>
      {/* Background layer */}
      <div className="absolute top-[10%] -left-[15%] w-[1200px] h-[1200px] opacity-[0.02] pointer-events-none -rotate-[15deg]">
        <img src="/PLA_logos-01.svg" alt="" className="w-full h-full object-contain" />
      </div>

      {/* Dark Section (Top) */}
      <div style={{ paddingTop: 140, paddingBottom: 80, position: 'relative', zIndex: 10 }}>
        <div className="section-container" style={{ maxWidth: 1120 }}>
          ...
          <div className="reveal visible" style={{ marginBottom: 56 }}>
            ...
            <h1 style={{ fontSize: 'clamp(2.25rem, 5vw, 3.5rem)', fontWeight: 800, color: '#ffffff', letterSpacing: '-0.04em', lineHeight: 1.08, marginBottom: 20 }}>
              Dynamic Brand &amp; interface <span className="gradient-text">style guide</span>
            </h1>
            <p style={{ color: '#7e99a8', fontSize: '1.0625rem', lineHeight: 1.75, maxWidth: 640 }}>
              Our v3 aesthetic embraces a high-end hybrid approach: blending the deep, rich intelligence of our dark mode with the clean, airy, and accessible feeling of our light mode.
            </p>
          </div>
          ...
          <section style={{ marginBottom: 80 }} className="reveal visible">
            <SectionTitle id="foundations" eyebrow="Strategy" title="Foundations">
              <p style={{ color: '#7e99a8', fontSize: '1rem', lineHeight: 1.75, maxWidth: 720 }}>
                We use an alternating layout strategy. The Hero and Team sections utilize a bright <code style={{ color: '#b8cdd6' }}>#fbfbf9</code> background to appear welcoming and modern, while the Narrative, Contact, and Footer plunge into deep <code style={{ color: '#b8cdd6' }}>#0c1a24</code> for an immersive "biological dark matter" aesthetic.
              </p>
            </SectionTitle>
          ...
          {/* Colours */}
          <section style={{ marginBottom: 80 }} className="reveal visible">
            <SectionTitle id="colors" eyebrow="Tokens" title="Core Colour System">
              <p style={{ color: '#7e99a8', fontSize: '0.9375rem', marginTop: 8 }}>
                The original dark tokens remain intact in <code style={{ color: '#b8cdd6' }}>index.css</code>. Tailwind allows us to swap these seamlessly depending on the active section background.
              </p>
            </SectionTitle>
            <div
              style={{
          ...
          <section style={{ marginBottom: 80 }} className="reveal visible">
            <SectionTitle id="typography" eyebrow="Type" title="Typography">
              <p style={{ color: '#7e99a8', fontSize: '0.9375rem', marginTop: 8 }}>
                <strong style={{ color: '#b8cdd6', fontWeight: 600 }}>Inter</strong> (300–900). 
              </p>
            </SectionTitle>
            <div className="glass-card" style={{ padding: 32, borderRadius: 16, marginBottom: 20 }}>
              ...
              <p style={{ fontSize: 'clamp(1rem, 1.5vw, 1.2rem)', fontWeight: 300, color: '#7e99a8', lineHeight: 1.8, letterSpacing: '0.01em', marginBottom: 16 }}>
                Subcopy pattern: lighter weight, silver, generous line-height.
              </p>
            </div>
      ...
      {/* Light Theme Section (Middle) */}
      <div style={{ backgroundColor: '#fbfbf9', position: 'relative', zIndex: 10, paddingTop: 100, paddingBottom: 100 }}>
        
        {/* Soft fluid canvas background simulation for light theme */}
        <div className="absolute top-[20%] right-0 w-[500px] h-[500px] bg-teal-500/5 rounded-full blur-[150px] pointer-events-none z-1" />
        <div className="absolute bottom-[20%] left-0 w-[600px] h-[600px] bg-orange-500/5 rounded-full blur-[160px] pointer-events-none z-1" />
        
        <div className="section-container" style={{ maxWidth: 1120, position: 'relative', zIndex: 10 }}>
          
          <section id="components" className="reveal visible">
            <SectionTitle id="components-light" eyebrow="New in v3" title="Light Theme UI" lightTheme={true}>
              <p style={{ color: '#64748b', fontSize: '0.9375rem', marginTop: 8 }}>Interactive primitives and content blocks adapted for our new airy sections (Hero & Team).</p>
            </SectionTitle>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginBottom: 32, padding: '8px 24px', borderRadius: 9999, border: '1px solid rgba(212,107,26,0.15)', background: 'rgba(255,255,255,0.8)', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#d46b1a', boxShadow: '0 0 12px rgba(212,107,26,0.6)' }} />
              <span style={{ color: '#475569', fontSize: '0.6875rem', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase' }}>Hero badge pattern</span>
            </div>

            <div className="bg-white border border-slate-200 shadow-sm" style={{ padding: 28, borderRadius: 16, maxWidth: 480 }}>
              <p style={{ color: '#0f172a', fontSize: '0.9375rem', fontWeight: 700, marginBottom: 16 }}>Light form field</p>
              <label style={{ color: '#64748b', fontSize: '0.8125rem', fontWeight: 600, display: 'block', marginBottom: 8 }}>Email Address *</label>
              <input
                readOnly
              ...
              />
              <p style={{ color: '#94a3b8', fontSize: '0.6875rem', marginTop: 16 }}>Focus states: border becomes orange, adds light orange ring.</p>
            </div>
          </section>
          ...
          {/* Headshot demo light */}
          <section style={{ marginBottom: 24 }} className="reveal visible">
            <h3 style={{ color: '#0f172a', fontSize: '1.125rem', fontWeight: 700, marginBottom: 20, letterSpacing: '-0.02em' }}>.team-photo-wrapper (Light Mode)</h3>
            <div className="bg-white border border-slate-200 hoverable shadow-sm" style={{ padding: 40, borderRadius: 20, maxWidth: 320, textAlign: 'center', transition: 'all 0.5s' }}>
              <div className="mx-auto mb-6 relative w-24 h-24 rounded-3xl p-px" style={{ background: 'linear-gradient(135deg, #d46b1a, #2dd4bf)' }}>
                <div className="w-full h-full rounded-3xl overflow-hidden bg-white">
                  <img src="/PLA_logos-02.svg" alt="" style={{ width: '100%', height: '100%', objectFit: 'contain', padding: '18%' }} />
                </div>
              </div>
              <p style={{ color: '#64748b', fontSize: '0.8125rem' }}>Gradient ring orange → teal; outer squircle radius with white core; shadow adds depth on hover.</p>
            </div>
          </section>
      ...
      {/* Dark Section (Bottom) */}
      <div style={{ paddingTop: 100, paddingBottom: 100, position: 'relative', zIndex: 10 }}>
        <div className="section-container" style={{ maxWidth: 1120 }}>
          
          {/* Surfaces */}
          <section style={{ marginBottom: 80 }} className="reveal visible">
            <SectionTitle id="surfaces" eyebrow="Depth" title="Dark Surfaces & global effects">
          ...
          <section style={{ marginBottom: 48 }} className="reveal visible">
            ...
            <div className="glass-card" style={{ padding: 28, borderRadius: 16 }}>
              ...
              <p style={{ color: '#3a5a6c', fontSize: '0.8125rem', marginTop: 20, lineHeight: 1.6 }}>
                Logo treatments: <code style={{ color: '#7e99a8' }}>.logo-platypus</code>, <code style={{ color: '#7e99a8' }}>.logo-platypus-hover</code>.
              </p>
            </div>
          </section>

          <div className="glass-card" style={{ padding: 32, borderRadius: 20, textAlign: 'center', marginTop: 80 }}>
