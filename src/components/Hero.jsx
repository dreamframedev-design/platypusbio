  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      reset() {
        ...
        this.baseOpacity = Math.random() * 0.5 + 0.3 // Significantly higher opacity
      ...
      update() {
        ...
        if (this.opacity >= this.baseOpacity + 0.3) this.pulseDir = -1
        if (this.opacity <= Math.max(0, this.baseOpacity - 0.2)) this.pulseDir = 1
      }
        ...
        <div className="w-full max-w-[720px] text-left p-8 md:p-12 lg:p-14 rounded-[2.5rem] backdrop-blur-2xl bg-white/40 border border-white/60 shadow-[0_32px_64px_rgba(0,0,0,0.15)] relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h1 className="animate-fade-in-up delay-300 text-[clamp(2.5rem,3.8vw,4rem)] font-black text-slate-900 leading-[1.05] tracking-[-0.04em] mb-[24px] drop-shadow-sm">
              ...
              <span className="bg-gradient-to-r from-[#d46b1a] to-[#e88430] bg-clip-text text-transparent drop-shadow-sm whitespace-nowrap">precision cell killing</span>
            </h1>
